...
public class PagingTimeAverageKPITest extends KPIBaseTestClass {

    private PagingTimeAverageKPI objUnderTest;

    @Before
    public void onSetUp(){
        objUnderTest = new PagingTimeAverageKPI();
    ...
    public void testgetKPICalculation() {
        final String expected = "max(isnull((case when isnull(suc.EVENT_ID, err.EVENT_ID)=13 and (isnull(suc.NO_OF_PAGING_ATTEMPTS, err.NO_OF_PAGING_ATTEMPTS)  > 0) then (cast (round((isnull((isnull(TOTAL_DURATION,0) )/cast(isnull(NO_OF_SUCCESSES,0) as DECIMAL(16)),0)), 2) as numeric(5,2))) end),0)) as 'Paging Time Average'";
