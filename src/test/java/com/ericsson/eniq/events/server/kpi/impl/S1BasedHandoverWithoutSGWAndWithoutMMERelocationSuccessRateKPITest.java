 ...
 *
 */
public class S1BasedHandoverWithoutSGWAndWithoutMMERelocationSuccessRateKPITest extends KPIBaseTestClass{

    private S1BasedHandoverWithoutSGWAndWithoutMMERelocationSuccessRateKPI objUnderTest;
    @Before
    public void onSetUp(){
        objUnderTest = new S1BasedHandoverWithoutSGWAndWithoutMMERelocationSuccessRateKPI(this.kpiUtilities);
    ...
    public void testgetKPICalculation() {
        final String expected = "max(isnull((case when isnull(err.EVENT_ID, suc.EVENT_ID)=7 and (isnull(suc.EVENT_SUBTYPE_ID, err.EVENT_SUBTYPE_ID)  = 4) then (cast (round((isnull((isnull(NO_OF_SUCCESSES,0) )/cast(isnull(NO_OF_ERRORS,0) + isnull(NO_OF_SUCCESSES,0) as DECIMAL(16)),0))*100, 2) as numeric(5,2))) end),0)) as 'S1 Based HO Without SGW And MME Relocation'";
