...
public class TrackingAreaUpdateSuccessRateKPITest extends KPIBaseTestClass {

    private TrackingAreaUpdateSuccessRateKPI objUnderTest;

    @Before
    public void onSetUp() {
        objUnderTest = new TrackingAreaUpdateSuccessRateKPI(this.kpiUtilities);
    ...
    public void testgetKPICalculation() {
        final String expected = "max(isnull((case when isnull(err.EVENT_ID, suc.EVENT_ID)=8 then (cast (round((isnull((isnull(NO_OF_SUCCESSES,0) )/cast(isnull(NO_OF_ERRORS,0) + isnull(NO_OF_SUCCESSES,0) as DECIMAL(16)),0))*100, 2) as numeric(5,2))) end),0)) as 'Tracking Area Update Success Rate'";
