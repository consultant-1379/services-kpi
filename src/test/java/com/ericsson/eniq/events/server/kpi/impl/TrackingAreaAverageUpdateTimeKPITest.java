...
public class TrackingAreaAverageUpdateTimeKPITest extends KPIBaseTestClass {
    ...
    public void testgetKPICalculation() {
        final String expected = "max(isnull((case when isnull(suc.EVENT_ID, err.EVENT_ID)=8 then (cast (round((isnull((isnull(TOTAL_DURATION,0) )/cast(isnull(NO_OF_SUCCESSES,0) as DECIMAL(16)),0)), 2) as numeric(5,2))) end),0)) as 'Tracking Area Average Update Time'\r\n";
        ...
        Assert.assertThat("KPI formula does not match", actual, Matchers.equalToIgnoringWhiteSpace(expected));
    }
}
