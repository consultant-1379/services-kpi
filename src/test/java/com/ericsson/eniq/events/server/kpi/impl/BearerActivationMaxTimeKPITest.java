...
public class BearerActivationMaxTimeKPITest extends KPIBaseTestClass {

    private BearerActivationMaxTimeKPI objUnderTest;

    @Before
    public void onSetUp(){
        objUnderTest = new BearerActivationMaxTimeKPI();
    ...
    public void testgetKPICalculation() {
        final String expected = "max(isnull((case when isnull(suc.EVENT_ID, err.EVENT_ID)=10 then isnull(MAX_DURATION, 0) end), 0)) as 'Bearer Activation Time Max'";
