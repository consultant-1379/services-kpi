...
public class PaginTimeMaxKPITest extends KPIBaseTestClass {

    private PagingTimeMaxKpi objUnderTest;

    @Before
    public void onSetUp(){
        objUnderTest = new PagingTimeMaxKpi();
    ...
    public void testgetKPICalculation() {
        final String expected = "max(isnull((case when isnull(suc.EVENT_ID, err.EVENT_ID)=13 and (isnull(suc.NO_OF_PAGING_ATTEMPTS, err.NO_OF_PAGING_ATTEMPTS)  >0) then isnull(MAX_DURATION, 0) end), 0)) as 'Maximum Paging Time'";
