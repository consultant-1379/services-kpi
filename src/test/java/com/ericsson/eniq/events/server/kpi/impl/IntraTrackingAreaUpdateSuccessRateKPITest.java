...
public class IntraTrackingAreaUpdateSuccessRateKPITest extends KPIBaseTestClass {

    private IntraTrackingAreaUpdateSuccessRateKPI objUnderTest;
    @Before
    public void onSetUp(){
        objUnderTest = new IntraTrackingAreaUpdateSuccessRateKPI(this.kpiUtilities);
    ...
    public void testgetKPICalculation() {
        final String expected = "max(isnull((case when isnull(err.EVENT_ID, suc.EVENT_ID)=8 and (isnull(suc.EVENT_SUBTYPE_ID, err.EVENT_SUBTYPE_ID)  = 1) then (cast (round((isnull((isnull(NO_OF_SUCCESSES,0) )/cast(isnull(NO_OF_ERRORS,0) + isnull(NO_OF_SUCCESSES,0) as DECIMAL(16)),0))*100, 2) as numeric(5,2))) end),0)) as 'Intra MME Tracking Area Update Success Rate'";
