...
public class PagingfailureRatioKPITest extends KPIBaseTestClass {

    private PagingFailureRatioKPI objUnderTest;

    @Before
    public void onSetUp() {
        objUnderTest = new PagingFailureRatioKPI(this.kpiUtilities);
    ...
    public void testgetKPICalculation() {
        final String expected = "max(isnull((case when isnull(err.EVENT_ID, suc.EVENT_ID)=13 and suc.NO_OF_PAGING_ATTEMPTS  > 0 and err.NO_OF_PAGING_ATTEMPTS  > 0 then (cast (round((isnull((isnull(NO_OF_ERRORS,0) )/cast(isnull(NO_OF_ERRORS,0) + isnull(NO_OF_SUCCESSES,0) as DECIMAL(16)),0))*100, 2) as numeric(5,2))) end),0)) as 'Paging Failure Ratio'";
